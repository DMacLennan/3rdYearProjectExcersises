...
length {A} Nil = zero -- yep!        -- {A} is now a hidden argument made explicit
...
{- 
   map  f     (Cons y (Cons y' (Cons y'' Nil)))
   = Cons (f y) (map f (Cons y' (Cons y'' Nil)))
   = Cons (f y) (Cons (f y') (map f  (Cons y'' Nil)))
   = Cons (f y) (Cons (f y') (cons (f y'') (map f Nil)))
   = Cons (f y) (Cons (f y') (cons (f y'') Nil))
-}
...
Nil ++ m = m
(Cons y y') ++ m = Cons y (y' ++ m) -- Done! Too easy!


{-   
 (Cons y (Cons y' Nil))  ++  (Cons z (Cons z' Nil))
=  Cons y (( Cons y' Nil) ++ (Cons z (Cons z' Nil))
=  Cons y ( Cons y'   (NIl ++ (Cons z (Cons z' Nil))  ))
= Cons  y   (Cons y'  (Cons z (Cons z' Nil)))
-}
...
vLength {n = n} v = n -- What's that all about? Doesn't Load... But...
                      -- Now it does!
